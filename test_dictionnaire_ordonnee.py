...
@parameterized_class(
    (
        ...
        "expected_keys",
        "expected_values",
        "expected_values_setitem",
    ),
    [
        (
            {"jil": 0, "emely": 8, "pierre": 3},
            {"jil": 0, "emely": 8, "pierre": 3},
            "jil",
            "emely",
            "pierre",
...
@parameterized_class(
    (
        ...
        "expected_add",
        "expected_sort",
        "expected_sort_reverse",
    ),
    [
        (
            {"jil": 0, "emely": 8, "pierre": 9},
            {"julien": 6, "elene": 8, "patrick": 18},
            ["{", "emely", ":", 0, "}"],
            3,
            {"jil": 0, "emely": 8, "pierre": 9, "julien": 6, "elene": 8, "patrick": 18},
            {"emely": 8, "jil": 0, "pierre": 9},
            {"pierre": 9, "jil": 0, "emely": 8},
        ),
        (
            {"jil": 0, "emely": 8},
            {"franck": 0, "emo": 4, "pierre": 9},
            '"emely":0',
            2,
            {"jil": 0, "emely": 8, "franck": 0, "emo": 4, "pierre": 9},
            {"emely": 8, "jil": 0},
            {"jil": 0, "emely": 8},
        ),
        (
            {"jil": 0, "emely": 8, "pierre": 9, "pat": 44, "vero": 34},
            {"jack": 89, "emelyne": 23, "paul": 97},
            1011091011081215848,
            5,
            {
                "jil": 0,
            ...
            {"emely": 8, "jil": 0, "pat": 44, "pierre": 9, "vero": 34},
            {"vero": 34, "pierre": 9, "pat": 44, "jil": 0, "emely": 8},
        ),
    ],
)
class TestStructuredDictionary(unittest.TestCase):
    def test_length(self):
        self.assertEqual(
            len(StructuredDictionary(self.dictionary1)), self.expected_length
        )

    def test_add(self):

        structured_dictionary1 = StructuredDictionary(
            self.dictionary1
        ) + StructuredDictionary(self.dictionary2)
        ...
        with self.assertRaises(TypeError):
            ...
            structured_dictionary_error = (
                structured_dictionary_error + self.fake_dictionary
            )

    def test_sort(self):
        self.assertEqual(
            json.loads(str(StructuredDictionary(self.dictionary1).sort())),
            self.expected_sort,
        )
        self.assertEqual(
            json.loads(str(StructuredDictionary(self.dictionary1).sort(reverse=True))),
            self.expected_sort_reverse,
        )


@parameterized_class(
    ("dictionary", "expected_representation"),
    [
...
class TestStructuredDictionaryRepresentation(unittest.TestCase):
    def test_representation(self):
        ...
        self.assertEqual(
            dictionary_representation.__str__(), self.expected_representation
        )


@parameterized_class(
    ("dictionary", "expected_keys", "expected_values"),
    [
        ({"jil": 0, "emely": 8, "pierre": 9}, ["jil", "emely", "pierre"], [0, 8, 9]),
        ({"jil": 0, "emely": 8}, ["jil", "emely"], [0, 8]),
        (
            {"jil": 0, "emely": 8, "pierre": 9, "pat": 44, "vero": 34},
            ["jil", "emely", "pierre", "pat", "vero"],
            [0, 8, 9, 44, 34],
        ),
    ],
)
