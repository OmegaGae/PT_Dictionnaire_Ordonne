...
# encapsulation des variables mentionnés ne sera pas fait car trop nombreux
class DictionnaireOrdonne:
    ...
    def __init__(self, dictionnaire=dict(), **parametresN_dict):
        ...
        if (type(dictionnaire) is not dict) and (
            type(dictionnaire) is not DictionnaireOrdonne
        ):
            raise TypeError(
    ...
    def __add__(self, objt_ajouter=dict()):
        ...
        _objt_ajouter_dict = json.loads(
            str(objt_ajouter)
        )  # petite triche car import json non vu dans le cours
        if (type(_objt_ajouter_dict) is not dict) and (
            type(_objt_ajouter_dict) is not DictionnaireOrdonne
        ):
            raise TypeError(
                "Cecin'est pas un dictionnaire, veuillez entrer un dictionnaire soit d"
                "type dict, ou DictionnaireOrdonne"
    ...
    def sort(self, reverse=False):
        ...
        if reverse == False:
            ...
            _tableau_sorted_cle = []
            _tableau_sorted_valeur = (
                self._liste_cle
            )  # none _liste_cle create contracdiction
